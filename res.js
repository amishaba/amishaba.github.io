...
ans.textContent=myusername+ "'s Reservations";
});
 ...
 function ajaxFetch(restaurantName, latitude, longitude, selectedRadioButton) {
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            displayData_(data,restaurantName);
        })
        .catch(error => {
  ...
  const clearme = document.getElementById("restaurantList");
  clearme.innerHTML="";
...
function submitReservation(date,time,curu,rest) {
	rest=res_rest;
	curu=myusername;
	console.log('SNEDING RES FOR'+curu+' '+myusername )
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 function fetch_res(choice) {
	 console.log("in fetch to get" +choice);
    // Get the base URL of the project root
    var baseUrl = window.location.origin + '/CSCI210_assn4/add_res'; // Replace 'your_project_directory' with the actual directory name
    console.log('baseUrl = '+baseUrl);
  ...
  restaurantList.style.marginTop="-390px";
  restaurantList.innerHTML="";
  ...
  data.forEach(restaurant => {
    ...
    dateElement.textContent = `Date: ${restaurant.date}`;
    
    const timeElement = document.createElement("p");
    timeElement.textContent = `Time: ${restaurant.time}`;
    ...
    // Style the paragraphs within the "r_info" div
    const paragraphs = [nameElement, addressElement, yelpElement,dateElement,timeElement ];
    paragraphs.forEach(paragraph => {
      paragraph.style.margin = "0";
      paragraph.style.fontSize = "16px";
      paragraph.style.marginBottom = "10px";
